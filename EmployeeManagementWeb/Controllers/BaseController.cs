...
namespace EmployeeManagement.Web.Controllers
{
    ...
    public abstract class BaseController : ControllerBase
    {
        ...
        protected async Task<IActionResult> ProcessRequest<TModel>(Func<Task<TModel>> func)
        {
            try
            {
                var str = await func();

                return Ok(new ApiResponse<TModel>(str));
            }
            catch (SqlException ex)
            {
                _logger.LogError(ex.Message);
                ...
                return BadRequest(new ApiResponse<TModel>(ex.Message));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
